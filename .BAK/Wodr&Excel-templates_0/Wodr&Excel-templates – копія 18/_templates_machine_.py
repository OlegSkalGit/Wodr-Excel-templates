...
def resolve_path(base_dir, path):
    ...
    cand2 = os.path.abspath(os.path.join(base_dir, os.path.basename(path)))
    if os.path.exists(cand2): return cand2
    cand3 = os.path.abspath(path)
    if os.path.exists(cand3): return cand3
    cand4 = os.path.abspath(os.path.basename(path))
...
def consolidate_paragraph_tags(p):
    ...
    for m in matches:
        ...
        if len(unique_runs) > 1:
            ...
            first_run.text = tag_text
            for other_run in unique_runs[1:]:
                other_run.text = ""
            char_to_run = []
            for r_idx, r in enumerate(p.runs):
                for _ in range(len(r.text)):
