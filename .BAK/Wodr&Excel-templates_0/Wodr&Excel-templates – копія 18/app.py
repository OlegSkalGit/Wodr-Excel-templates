...
def extract_placeholders_with_context(template_path):
    """Extracts jinja2 placeholders like {{var}} from a docx or xlsx template along with their context."""
    import re
    import openpyxl
    from docx import Document
                    ...
                    for cell in r:
                        ...
                        if isinstance(val, str):
                            ...
                            for match in matches:
                                ...
                                context = val[start_idx:end_idx].strip()
                                placeholders[var_name] = f"Комірка {cell.coordinate}: {context}"
        except Exception as e:
            st.error(f"Помилка зчитування Excel: {e}")
            
    return placeholders

def run_heuristic_auto_rename(template_path, current_headers):
    """Fallback Ukrainian-context-aware heuristic auto-renamer for local execution without LLM AVX dependencies."""
    placeholders = extract_placeholders_with_context(template_path)
    if not placeholders:
        return {}
    suggestions = {}
    rules = [
        (["прізвищ", "ім'я", "по батьков", "піб", "фіо", "ф.и.о.", "особа", "громадян"], "full_name"),
        (["паспорт", "серія", "виданий"], "passport_details"),
        (["код", "ідентифікац", "іпн", "іпн", "інн", "инн"], "tax_code"),
        (["єдрпоу", "едрпоу"], "edrpou_code"),
        (["адрес", "проживан", "реєстрац", "вулиця", "будинок", "квартира", "місто", "область"], "address"),
        (["телефон", "тел.", "мобільн"], "phone_number"),
        (["пошта", "email", "e-mail", "mail"], "email_address"),
        (["сума", "гривень", "грн", "копійок", "коп", "вартість", "ціна", "сплатити"], "amount"),
        (["рахунок", "р/р", "iban", "ібан", "мфо", "банк"], "bank_details"),
        (["дата", "число", "місяць", "рік", "202", "р."], "document_date"),
        (["договір", "контракт", "угода", "№", "номер"], "document_number"),
        (["посада", "директор", "керівник", "голова", "начальник"], "job_title"),
        (["організац", "підприємств", "товариств", "тов", "пп", "фірма", "компанія"], "organization_name"),
        (["підпис", "ініціал"], "signature_name"),
    ]
    for var, context in placeholders.items():
        ctx_lower = context.lower()
        suggested = None
        for keywords, sugg_name in rules:
            if any(kw in ctx_lower for kw in keywords):
                suggested = sugg_name
                break
        if not suggested:
            clean_var = "".join([c if c.isalnum() or c == "_" else "_" for c in var]).strip("_").lower()
            if clean_var and not clean_var.startswith("field"):
                suggested = clean_var
            else:
                suggested = f"variable_{var}"
        base_sugg = suggested
        counter = 1
        while suggested in suggestions.values() or suggested in current_headers:
            suggested = f"{base_sugg}_{counter}"
            counter += 1
        suggestions[var] = suggested
    return suggestions

def run_local_llm_auto_rename(template_path, current_headers):
    """Uses the local Qwen model via Hugging Face Transformers to intelligently rename variables, falling back to heuristics on errors."""
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer
    import os
    
    local_model_path = os.path.join("models", "qwen_local")
    
    def fallback_heuristics(reason_msg):
        st.warning(f"⚠️ {reason_msg} Система автоматично перемикається на розумний евристичний аналізатор контексту!")
        return run_heuristic_auto_rename(template_path, current_headers)

    if not os.path.exists(local_model_path):
        return fallback_heuristics("Локальна папка з моделлю qwen_local не знайдена.")
        
    placeholders = extract_placeholders_with_context(template_path)
    if not placeholders:
        st.info("У шаблоні не знайдено жодної змінної у форматі {{імя_змінної}}!")
        return None
        
    st.info("🧠 Локальна модель завантажується в оперативну пам'ять (це займе кілька секунд)...")
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            local_model_path,
            torch_dtype=torch.float32,  # float32 для обходу AVX інструкцій
            device_map="cpu",
            local_files_only=True
        )
        
        prompt_lines = []
        prompt_lines.append("You are an expert assistant for document automation. Analyze the variables in the template and suggest descriptive, concise, snake_case variable names in Ukrainian or English depending on context. Return only the valid JSON object containing mappings from the original name to the suggested name.")
        prompt_lines.append("Variables to analyze with their context:")
        
        for var, context in placeholders.items():
            prompt_lines.append(f"- '{var}': context: \"... {context} ...\"")
            
        prompt_lines.append("\nJSON format required: {\"original_name\": \"suggested_name\"}. Return ONLY the JSON block, no markdown, no conversational text.")
        ...
        for k, v in suggestions.items():
            clean_v = "".join([c if c.isalnum() or c == "_" else "_" for c in v]).strip("_").lower()
            if clean_v and clean_v != k:
                valid_suggestions[k] = clean_v
                
        return valid_suggestions
        
    except Exception as e:
        err_msg = str(e)
        return fallback_heuristics(f"Помилка роботи з LLM через Transformers: {err_msg}.")
...
with st.sidebar.expander("⚙️ Налаштування ШІ (AI Settings)", expanded=True):
    st.markdown("##### 🤖 Модель для авто-перейменування")
    st.markdown("**Модель:** `Qwen 2.5 0.5B` *(Надзвичайно легка)*")
    
    local_model_path = os.path.join("models", "qwen_local")
    st.session_state["active_model_path"] = local_model_path
    
    # Show status
    if os.path.exists(local_model_path):
        st.success("✅ Модель завантажена локально")
    else:
        st.warning("⚠️ Модель не завантажена")
        
        # Download button
        if st.button("📥 Завантажити модель", key="btn_download_model"):
            with st.spinner("📥 Завантаження та збереження моделі з Hugging Face (це може зайняти кілька хвилин)..."):
                try:
                    from transformers import AutoModelForCausalLM, AutoTokenizer
                    ...
                    st.success("🎉 Модель успішно завантажено та збережено!")
                    st.rerun()
                except Exception as e:
                    st.error(f"Помилка завантаження: {e}")
                
    st.markdown("##### 💻 Сумісність процесора")
    st.success("🤖 Встановлено CPU-сумісну версію через PyTorch (інструкції AVX2 не потрібні).")
...
with tab_editor:
    ...
    if not cfg_path:
        ...
        if sheets_data:
            ...
            with col_ctrl1:
                with st.popover("➕ Додати стовпчик (змінну)", use_container_width=True):
                    new_col = st.text_input("Ім'я нової змінної (напр., client_name):", key="compact_new_col_input")
                    if st.button("➕ Додати стовпчик", key="btn_compact_add_col", use_container_width=True):
                        if not new_col:
            ...
            with col_ctrl2:
                with st.popover("❌ Видалити стовпчик", use_container_width=True):
                    if st.session_state["current_sheet_headers"]:
                        ...
                        if st.button("❌ Видалити стовпчик", key="btn_compact_del_col", type="primary", use_container_width=True):
                            if not confirm_col:
            ...
            with col_ctrl3:
                if st.button("➕ Додати рядок", key="btn_compact_add_row", use_container_width=True):
            ...
            with col_ctrl4:
                with st.popover("❌ Видалити рядок", use_container_width=True):
                    if st.session_state["current_sheet_data"]:
                        ...
                        if st.button("❌ Видалити рядок", key="btn_compact_del_row", type="primary", use_container_width=True):
                            if not confirm_row:
                                st.error("Будь ласка, підтвердіть видалення!")
                            else:
            ...
            with col_save2:
                # --- LOCAL LLM AUTO-RENAME BUTTON ---
                if st.button("🤖 Авто-перейменувати змінні локальною LLM", key="btn_llm_auto_rename", use_container_width=True):
                    template_path = st.session_state["editor_template_path"]
                    ...
                    if not template_path:
                        st.error("Будь ласка, вкажіть шлях до файлу шаблону в полі A1!")
                    elif not os.path.exists(actual_template_path):
                        st.error(f"Файл шаблону не знайдено за шляхом: {actual_template_path}!")
                    else:
                        local_model_path = os.path.join("models", "qwen_local")
                        if not os.path.exists(local_model_path):
                            st.session_state["show_llm_download_prompt"] = True
                            st.session_state["llm_actual_template_path"] = actual_template_path
                        else:
                            with st.spinner("🤖 Локальна модель аналізує ваш шаблон..."):
                                suggestions = run_local_llm_auto_rename(actual_template_path, headers)
                                if suggestions:
                                    st.session_state["llm_suggestions"] = suggestions
                                    st.success("🤖 Модель згенерувала пропозиції щодо перейменування!")
                                else:
                                    st.info("Локальна LLM не знайшла змінних для перейменування або всі запропоновані імена вже існують.")
            
            # --- MODEL DOWNLOAD PROMPT ---
            if st.session_state.get("show_llm_download_prompt", False):
                st.info("ℹ️ Локальна LLM-модель Qwen 2.5 0.5B відсутня на вашому диску. Для роботи цієї функції необхідно завантажити файли моделі (~468 МБ).")
                col_dl1, col_dl2 = st.columns([1, 1])
                with col_dl1:
                    if st.button("📥 Завантажити модель зараз", key="btn_download_llm_model", use_container_width=True):
                        with st.spinner("📥 Завантаження та збереження моделі з Hugging Face (це може зайняти кілька хвилин)..."):
                            try:
                                from transformers import AutoModelForCausalLM, AutoTokenizer
                                model_name = "Qwen/Qwen2.5-0.5B-Instruct"
                                ...
                                model.save_pretrained(local_model_path)
                                st.success("✅ Модель успішно завантажено та збережено!")
                                st.session_state["show_llm_download_prompt"] = False
                                st.rerun()
                            except Exception as ex:
                                st.error(f"❌ Помилка завантаження моделі: {ex}")
                with col_dl2:
                    if st.button("💡 Запустити швидку евристику", key="btn_run_heuristics_fallback", use_container_width=True):
                        actual_template_path = st.session_state.get("llm_actual_template_path")
                        if actual_template_path and os.path.exists(actual_template_path):
                            with st.spinner("💡 Запуск евристичного аналізатора..."):
                                suggestions = run_heuristic_auto_rename(actual_template_path, headers)
                                if suggestions:
                                    st.session_state["llm_suggestions"] = suggestions
                                    st.success("💡 Евристичний аналізатор згенерував пропозиції!")
                                else:
                                    st.info("Евристичний аналізатор не знайшов змінних для перейменування.")
                        st.session_state["show_llm_download_prompt"] = False
                        st.rerun()
            
            # --- RENDER LLM SUGGESTIONS IF PRESENT ---
            if "llm_suggestions" in st.session_state and st.session_state["llm_suggestions"]:
                st.write(" ")
                st.markdown("##### 💡 Пропозиції локальної LLM щодо перейменування змінних:")
                suggestions = st.session_state["llm_suggestions"]
                
                for orig, sugg in list(suggestions.items()):
                    col_s1, col_s2, col_s3 = st.columns([2, 2, 1])
                    with col_s1:
                        st.text_input("Поточне ім'я:", value=orig, disabled=True, key=f"orig_sugg_{orig}")
                    with col_s2:
                        new_sugg = st.text_input("Пропоноване ім'я (snake_case):", value=sugg, key=f"new_sugg_{orig}")
                        suggestions[orig] = new_sugg
                    with col_s3:
                        st.write(" ")
                        st.write(" ")
                        if st.button("✅ Застосувати", key=f"btn_apply_sugg_{orig}"):
                            if new_sugg in st.session_state["current_sheet_headers"]:
                                st.error("Така змінна вже існує!")
                            else:
                                # Rename header in config
                                h_idx = st.session_state["current_sheet_headers"].index(orig)
                                st.session_state["current_sheet_headers"][h_idx] = new_sugg
                                for row in st.session_state["current_sheet_data"]:
                                    if orig in row:
                                        row[new_sugg] = row.pop(orig)
                                
                                # Rename placeholder in template file as well
                                ...
                                if t_path:
                                    ...
                                    if os.path.exists(actual_t_path):
                                        if rename_placeholder_in_template(actual_t_path, orig, new_sugg):
                                            st.toast(f"✅ Шаблон також оновлено (замінено {{{{{orig}}}}} на {{{{{new_sugg}}}}})", icon="📄")
                                
                                del suggestions[orig]
                                st.session_state["llm_suggestions"] = suggestions
                                st.success(f"Змінну перейменовано на '{new_sugg}'!")
                                st.rerun()
