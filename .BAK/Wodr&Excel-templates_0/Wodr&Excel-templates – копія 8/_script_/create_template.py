...
def create_main_excel(path, template_path, vars_old, vars_new, batch_data=None):
...
def extract_batch(f1, f2, scan_dir, ext, diff_info):
    results, files = [], []
    abs_f1, abs_f2 = os.path.abspath(f1), os.path.abspath(f2)
    for root, _, fs in os.walk(scan_dir):
        for f in fs:
            if f.lower().endswith(ext):
                path = os.path.abspath(os.path.join(root, f))
                if path != abs_f1 and path != abs_f2: files.append(path)
    if not files: return []
    print(f"Пакетна обробка {len(files)} файлів...")
    for f_path in files:
        try:
            entry = {}
            if ext == '.xlsx':
                ...
                for (s_idx, r, c, var_name) in diff_info:
                    if s_idx < len(wb.worksheets):
                        ...
                        if r == -1: # Назва аркуша
                            v1, v_cur = ws1.title, ws.title
                        else: # Комірка
                            v1, v_cur = ws1.cell(row=r, column=c).value, ws.cell(row=r, column=c).value
...
def compare_excel(f1, f2, main_xls, template_out, scan_dir=None):
    ...
    vars_old, vars_new, diff_map, v_idx, diff_locations = {}, {}, {}, [1], []
    for i in range(min(len(wb1_d.sheetnames), len(wb2_d.sheetnames))):
        s1_n, s2_n = wb1_d.sheetnames[i], wb2_d.sheetnames[i]
        nv, mod, local = smart_diff_replace(s1_n, s2_n, diff_map, vars_old, vars_new, v_idx)
        if mod:
            wb_t.worksheets[i].title = nv
            for (vn, _) in local: diff_locations.append((i, -1, -1, vn))
        ...
        for r in range(1, max(s1.max_row, s2.max_row) + 1):
            for c in range(1, max(s1.max_column, s2.max_column) + 1):
                ...
                if v1 != v2:
                    ...
                    if mod:
                        ct.value = nv
                        for (vn, _) in local: diff_locations.append((i, r, c, vn))
    batch_res = None
    if scan_dir: batch_res = extract_batch(f1, f2, scan_dir, '.xlsx', diff_locations)
    t_p = get_unique_path(template_out)
    wb_t.save(t_p)
    m_p = create_main_excel(main_xls, t_p, vars_old, vars_new, batch_res)
    print(f"\nУспішно!\nШаблон: {t_p}\nКонфіг: {m_p}")

def compare_word(f1, f2, main_xls, template_out, scan_dir=None):
            ...
            for j in range(min(len(g1), len(g2))):
                ...
                if t1 != t2:
                    ...
                    if mod:
                        ...
                        for (vn, _) in local: diff_locations.append(('p', i, j, vn))
        ...
        for r in range(min(len(t1.rows), len(t2.rows))):
            for c in range(min(len(t1.columns), len(t2.columns))):
                ...
                if c1.text != c2.text:
                    ...
                    if mod:
                        ct.text = nv
                        for (vn, _) in local: diff_locations.append(('t', t_i, (r, c), vn))
    batch_res = None
    if scan_dir: batch_res = extract_batch(f1, f2, scan_dir, '.docx', diff_locations)
    t_p = get_unique_path(template_out)
    doc_t.save(t_p)
    m_p = create_main_excel(main_xls, t_p, vars_old, vars_new, batch_res)
    print(f"\nУспішно!\nШаблон: {t_p}\nКонфіг: {m_p}")
...
def main():
    if len(sys.argv) < 3:
        print("\nВикористання: python create_template.py <старий> <новий> [каталог_сканування]")
        return
    f1, f2 = sys.argv[1:3]
    ...
    try:
        if ex == '.xlsx': compare_excel(f1, f2, m_x, t_o, scan_dir)
        elif ex == '.docx': compare_word(f1, f2, m_x, t_o, scan_dir)
