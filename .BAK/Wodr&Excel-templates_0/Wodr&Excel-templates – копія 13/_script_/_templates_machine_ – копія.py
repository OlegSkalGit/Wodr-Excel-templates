...
def main():
    ...
    if arg1.lower().endswith('.xlsx') and os.path.isfile(arg1):
        if len(args) == 2 and os.path.isfile(args[1]) and args[1].lower().endswith('.xlsx'):
    ...
    if len(args) == 2 and os.path.isfile(arg1):
        arg2 = args[1]
        if os.path.isfile(arg2):
            run_compare_two(arg1, arg2)
        elif os.path.isdir(arg2):
            run_package(arg1, arg2)
        else:
            print("Помилка: Другий параметр має бути існуючим файлом або папкою.")
        return
