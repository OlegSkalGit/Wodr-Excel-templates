...
def render_config_editor(cfg_path):
    ...
    sheet_names = list(sheets_data.keys()) if sheets_data else []
    ...
    # Safely get sheet_info even if sheets_data is empty
    sheet_info = sheets_data.get(selected_sheet, {
        "template_path": "",
        "name_pattern": "",
        "headers": [],
        "rows": []
    ...
    if st.session_state.get("loaded_config_sheet") != config_sheet_key:
        ...
        # Only overwrite paths from file if the file actually exists and was read successfully
        if sheets_data:
    ...
    with st.expander("✏️ Редагувати назви змінних (конфіг та шаблон)", expanded=False):
        ...
        headers_df = pd.DataFrame({
            "Поточне ім'я змінної": pd.Series(headers_list, dtype=object),
            "Нова назва змінної": pd.Series(headers_list, dtype=object)
        })
